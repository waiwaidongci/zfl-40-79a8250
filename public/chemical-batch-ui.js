  ...
  function showDetail(id) {
    ...
    modal.innerHTML = `
      <div class="panel" style="width:480px;max-height:80vh;overflow:auto;position:relative">
        ...
        <div id="batchNegativesList">
          ${(b.negativeCodes || []).length
            ? b.negativeCodes.map(code => `<div class="meta" style="padding:4px 0;border-bottom:1px solid var(--line)">${code}</div>`).join('')
            : '<div class="meta">暂无关联底片</div>'
          }
        </div>
      </div>
    `;
